...
<style>
  ...
  /* Compact semantic row classes avoid repeating utility strings in SSR HTML. */
  .problem-row {
    position: relative;
    overflow: hidden;
  ...
  .problem-row:not(.problem-row-solved):hover {
    background: color-mix(in oklab, var(--color-tertiary) 30%, transparent);
  }

  .problem-row-solved {
    border-left: 4px solid var(--color-solved);
    background: var(--color-solved-row);
  }
